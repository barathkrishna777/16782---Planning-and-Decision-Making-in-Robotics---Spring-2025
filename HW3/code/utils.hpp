...
struct ConditionHasher
{
    size_t operator()(const Condition& cond) const {
        size_t hash_value = std::hash<std::string>{}(cond.get_predicate());
        for (const auto& arg : cond.get_args()) {
            hash_value ^= std::hash<std::string>{}(arg) + 0x9e3779b9 + (hash_value << 6) + (hash_value >> 2);
        }
        return hash_value;
    }
};
...
public:
    ...
    bool operator==(const Action& rhs) const
    {
        return 
            this->name == rhs.name &&
            this->args == rhs.args &&
            this->preconditions == rhs.preconditions &&
...
struct ActionHasher
{
    size_t operator()(const Action& ac) const
    {
        ...
        for (char c : ac.get_name()) {
            hash_value = hash_value * 31 + c;
        }
        for (const auto& arg : ac.get_args()) {
            for (char c : arg) {
                hash_value = hash_value * 31 + c;
            }
        }
        for (const auto& pre : ac.get_preconditions()) {
            hash_value = hash_value * 31 + ConditionHasher{}(pre);
        }
        for (const auto& eff : ac.get_effects()) {
            hash_value = hash_value * 31 + ConditionHasher{}(eff);
        }
...
private:
    ...
    unordered_set<Action, ActionHasher> actions;
...
public:
    ...
    unordered_set<Action, ActionHasher> get_actions() const {
        return this->actions;
    }
